...
# Prepare data
y = df_h1['y']
# Use A1-A64 naming (fixed from Attr1-Attr64)
feature_cols = [col for col in df_h1.columns 
                if col.startswith('A') and col[1:].split('_')[0].isdigit() and not col.endswith('__isna')]
...
vif_all = calculate_vif(X_train_vif[list(X_train.columns)])
vif_selected = calculate_vif(X_train_vif[remaining_features])
vif_all.to_csv(output_dir / 'vif_all_features.csv', index=False)
vif_selected.to_csv(output_dir / 'vif_selected_features.csv', index=False)
print(f"  ✓ Saved VIF results to CSV files")
...
try:
    ...
    model_white = sm.Logit(y_train, X_train_white).fit(disp=0, cov_type='HC3', maxiter=100)  # HC3 = White robust SE
    ...
    print(f"  ⚠️  Warning: Could not fit White robust SE model ({str(e)})")
    print(f"  Using VIF-selected features instead")
    auc_white = auc_vif
    results_summary['methods']['white_robust_se'] = {
        'test_auc': float(auc_white),
        'note': f'Skipped due to numerical issue: {str(e)}. Use VIF-selected features.',
        'features': remaining_features
    }
    remediated_datasets['white_robust'] = (df_vif, remaining_features, auc_white)
