#!/usr/bin/env python3
"""
Script 02b: VIF Multicollinearity Remediation - American Dataset
...
print("=" * 80)
print("SCRIPT 02b: VIF MULTICOLLINEARITY REMEDIATION - AMERICAN DATASET")
print("=" * 80)
...
output_dir = PROJECT_ROOT / 'results' / 'script_outputs' / '02_american' / '02b_vif_remediation'
...
if low_vif_features:
    print(f"  Features: {', '.join(low_vif_features)}")
...
# Iterative VIF removal (if no features < 5)
if not low_vif_features:
    print("\n[4/6] Method 2: Iterative VIF Removal (threshold = 10)...")
    ...
    while True:
        ...
        if max_vif < 10 or len(X_temp.columns) <= 5:
            break
        ...
        if iteration % 5 == 0:
            print(f"    Iteration {iteration}: Removed {len(removed)} features, max VIF = {max_vif:.1f}")
    
    low_vif_features = X_temp.columns.tolist()
    print(f"\n  Removed: {len(removed)} features")
    print(f"  Remaining: {len(low_vif_features)} features")
    print(f"  Max VIF: {calculate_vif_all(X_temp)['vif'].max():.2f}")
    
    epv_vif = y.sum() / len(low_vif_features) if low_vif_features else 0
    print(f"  EPV: {epv_vif:.2f}")

# Forward selection for optimal predictive subset
print(f"\n[{4 if not low_vif_features else 5}/6] Method 3: Forward Selection...")
...
if low_vif_features:
    ...
    forward_features = []
    print("  ⚠️ Skipped (no low VIF features available)")

# Save all results
print(f"\n[{5 if not low_vif_features else 6}/6] Saving results...")
...
if low_vif_features:
    ...
    df_vif.to_parquet(data_dir / 'american_vif_selected.parquet')
    print(f"  ✓ {data_dir / 'american_vif_selected.parquet'}")
...
if forward_features:
    ...
    df_forward.to_parquet(data_dir / 'american_forward_selected.parquet')
    print(f"  ✓ {data_dir / 'american_forward_selected.parquet'}")
...
# Final summary
print("\n" + "=" * 80)
print("AMERICAN DATASET - VIF REMEDIATION COMPLETE")
print("=" * 80)
print(f"""
...
Recommended Feature Sets:
  1. VIF-selected ({len(low_vif_features) if low_vif_features else 0} features): EPV = {epv_vif if low_vif_features else 0:.1f}
     File: american_vif_selected.parquet
     Features: {', '.join(low_vif_features[:10]) if low_vif_features else 'NONE'}{'...' if low_vif_features and len(low_vif_features) > 10 else ''}
  ...
  2. Forward-selected ({len(forward_features) if forward_features else 0} features): EPV = {forward_epv if forward_features else 0:.1f}
     File: american_forward_selected.parquet
     Features: {', '.join(forward_features) if forward_features else 'N/A'}
